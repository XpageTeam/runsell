...
document.addEventListener("DOMContentLoaded", function () {
	...
	$(".filter__item-title").click(function(){
		const $this = $(this),
			$curFilterItem = $this.closest(".filter__item");

		// if ($curFilterItem.hasClass(".filter__item--closed")){
		// 	$curFilterItem.find(".filter__item-content").slideDown(300);
		// }else{
		// 	$curFilterItem.find(".filter__item-content").slideUp(300);
		// }
		$curFilterItem.find(".filter__item-content").slideToggle(300);
		$curFilterItem.toggleClass("filter__item--closed");
	});
...
setMinHandle = ($uiSlider, count) => {
	$uiSlider.find(".price-filter__currents-form").text(`от ${partNumber(count)} руб`)
},
setMaxHandle = ($uiSlider, count) => {
	$uiSlider.find(".price-filter__currents-to").text(`до ${partNumber(count)} руб`)
}